  ...
  return <div className="space-y-8">
    {blocks.map((block, index) => {
      ...
      if (block.type === "gallery") {
        return <div key={`${block.type}-${index}`} className={`grid gap-4 ${block.images.length > 1 ? "sm:grid-cols-2" : ""}`}><div className={`sm:col-span-full border-b pb-3 font-mono text-[7px] uppercase tracking-[.18em] ${light ? "border-[#294368]/10 text-[#294368]/35" : "border-white/[.08] text-white/26"} ${rtl ? "text-right [direction:rtl]" : "text-left"}`}>BLOCK / {blockNumber} / GALLERY / {String(block.images.length).padStart(2, "0")} ITEMS</div>{block.images.map((image, imageIndex) => <figure key={`${image.src}-${imageIndex}`} className={`overflow-hidden rounded-[20px] border ${light ? "border-[#294368]/10 bg-white/45" : "border-white/[.07] bg-white/[.018]"}`}><div className="relative aspect-[4/3] overflow-hidden"><Image src={image.src} alt={language === "fa" ? image.altFa : image.alt} fill sizes="(max-width: 900px) 100vw, 600px" className="object-cover transition-transform duration-700 hover:scale-[1.02]" /></div>{(image.caption || image.captionFa) && <figcaption className={`border-t px-4 py-3 font-mono text-[8px] uppercase tracking-[.14em] ${light ? "border-[#294368]/10 text-[#294368]/42" : "border-white/[.07] text-white/30"} ${rtl ? "text-right [direction:rtl]" : "text-left"}`}>{text(image.caption ?? "", image.captionFa ?? image.caption ?? "")}</figcaption>}</figure>)}</div>;
      }
      if (block.type === "quote") {
        return <blockquote key={`${block.type}-${index}`} className={`relative overflow-hidden rounded-[22px] border p-[clamp(24px,4vw,42px)] ${light ? "border-[#294368]/10 bg-white/52" : "border-white/[.07] bg-white/[.018]"} ${rtl ? "text-right [direction:rtl]" : "text-left"}`}><i className="absolute left-0 top-0 h-full w-1" style={{ backgroundColor: accent }} /><span className={`mb-6 block font-mono text-[7px] uppercase tracking-[.18em] ${light ? "text-[#294368]/32" : "text-white/25"}`}>BLOCK / {blockNumber} / QUOTE</span><p className="max-w-[48ch] text-[clamp(20px,2.7vw,34px)] font-[520] leading-[1.12] tracking-[-.035em]">“{text(block.text, block.textFa)}”</p>{(block.byline || block.bylineFa) && <cite className={`mt-6 block font-mono text-[8px] uppercase tracking-[.17em] not-italic ${light ? "text-[#294368]/40" : "text-white/30"}`}>{text(block.byline ?? "", block.bylineFa ?? block.byline ?? "")}</cite>}</blockquote>;
      }
      if (block.type === "video") {
        return block.src ? <figure key={`${block.type}-${index}`} className={`overflow-hidden rounded-[24px] border ${light ? "border-[#294368]/10 bg-white/45" : "border-white/[.07] bg-white/[.018]"}`}><video src={block.src} poster={block.poster} controls className="aspect-video w-full object-cover" />{(block.caption || block.captionFa) && <figcaption className={`border-t px-5 py-3 font-mono text-[8px] uppercase tracking-[.14em] ${light ? "border-[#294368]/10 text-[#294368]/42" : "border-white/[.07] text-white/30"}`}>{text(block.caption ?? "", block.captionFa ?? block.caption ?? "")}</figcaption>}</figure> : <div key={`${block.type}-${index}`} className={`flex min-h-[220px] flex-col items-center justify-center rounded-[24px] border border-dashed p-8 text-center ${light ? "border-[#294368]/20 text-[#294368]/40" : "border-white/[.12] text-white/30"}`}><span className="font-mono text-[9px] uppercase tracking-[.16em]">{copy.videoPlaceholder}</span><span className={`mt-3 max-w-[30ch] text-[11px] leading-6 ${light ? "text-[#294368]/50" : "text-white/38"}`}>{copy.contentEmpty}</span></div>;
      }
      return block.url ? <a key={`${block.type}-${index}`} href={block.url} target="_blank" rel="noreferrer" className={`group flex min-h-[110px] items-center justify-between rounded-[22px] border px-5 transition-transform hover:-translate-y-1 ${light ? "border-[#294368]/10 bg-white/52" : "border-white/[.07] bg-white/[.018]"}`}><div><span className={`block font-mono text-[7px] uppercase tracking-[.18em] ${light ? "text-[#294368]/35" : "text-white/27"}`}>BLOCK / {blockNumber} / EMBED</span><span className="mt-3 block text-[13px] font-medium">{text(block.label, block.labelFa)}</span></div><ArrowUpRight className="h-5 w-5 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" strokeWidth={1.4} /></a> : <div key={`${block.type}-${index}`} className={`flex min-h-[110px] flex-col items-center justify-center rounded-[22px] border border-dashed p-6 text-center ${light ? "border-[#294368]/20 text-[#294368]/40" : "border-white/[.12] text-white/30"}`}><span className="font-mono text-[9px] uppercase tracking-[.16em]">{copy.embedPlaceholder}</span><span className={`mt-3 text-[11px] ${light ? "text-[#294368]/50" : "text-white/38"}`}>{copy.contentEmpty}</span></div>;
    })}
  </div>;
}
      ...
      <article className="relative z-10">
        ...
        <section data-project-reveal className="mx-auto grid w-[min(1240px,calc(100%_-_clamp(28px,8vw,128px)))] gap-10 py-[clamp(90px,12vw,170px)] lg:grid-cols-[.34fr_1fr]">
          ...
          <div className={rtl ? "text-right [direction:rtl]" : "text-left"}>
            ...
            <dl className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 [direction:ltr]">
